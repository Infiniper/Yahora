    ...
    >
      <div className={styles.imageWrap}>
        <div className={styles.imageTrack} style={{ transform: `translateX(-${imgIndex * 100}%)` }}>
          {images.map((url, i) => (
            <img key={i} src={url} alt={`${product.title} ${i + 1}`} className={styles.image} loading={i === 0 ? "eager" : "lazy"} draggable={false} />
          ))}
        </div>
        {totalImages > 1 && (
          <>
        ...
        <button className={`${styles.overlayHeart} ${product.is_liked ? styles.overlayHeartLiked : ""}`} onClick={handleLike}>
      ...
      <div className={styles.infoWrap}>
        <div className={styles.metaRow}>
          <div className={styles.metaLeft}>
            <span className={styles.conditionBadge} style={{ background: condCfg.bg, color: condCfg.color }}>
              {condCfg.label}
            </span>
          </div>
          <span className={styles.price}>{displayPrice}</span>
        </div>
        {product.location && <span className={styles.locationTag}>{product.location.toUpperCase()}</span>}
        <h3 className={styles.title}>{product.title}</h3>

        <div className={styles.footerRow}>
          <div className={styles.stats}>
            <span className={styles.stat}>
              <EyeIcon /><span>{viewCount}</span>
            </span>
            
            {/* UPDATED: Rely on product.is_liked prop */}
            <button className={`${styles.statBtn} ${product.is_liked ? styles.statBtnLiked : ""}`} onClick={handleLike}>
            ...
            <button className={`${styles.statBtn} ${product.is_saved ? styles.statBtnSaved : ""}`} onClick={handleSave}>
          ...
          {isOwner ? (
            <div className={styles.ownerActions}>
               {product.status === "sold" && product.sold_to ? (
                  <span className={styles.cardSoldTo}>SOLD TO @{product.sold_to}</span>
               ) : (
                  <>
                    ...
                    <button className={`${styles.cardBtn} ${styles.cardBtnDelete}`} title="Delete listing" onClick={(e) => { e.stopPropagation(); onDelete?.(); }} style={{ color: '#f87171' }}>
                      <TrashIcon size={14} />
                    </button>
                  </>
               )}
