...
/* ─────────────────────────────────────────────
  SUB-COMPONENTS
───────────────────────────────────────────── */
function Avatar({ src, name, size, editable, onUpload, onRemove, isUploading }) {
 ...
 const initials = (name || "User")
   ...
   .slice(0, 2)
   .toUpperCase();
 const hue = (name || "User").split("").reduce((acc, c) => acc + c.charCodeAt(0), 0) % 360;

 return (
   <div className={styles.avatarWrap} style={{ "--av-size": size + "px" }}>
     <div className={styles.avatarRing}>
       {isUploading ? (
         <div className={styles.avatarPlaceholder} style={{ background: "#eee", color: "#888", fontSize: "14px" }}>
           Wait...
         </div>
       ) : src ? (
         <img src={src} alt="avatar" className={styles.avatarImg} />
       ) : (
         <div className={styles.avatarPlaceholder} style={{ background: `linear-gradient(135deg, hsl(${hue}, 60%, 55%), hsl(${hue + 40}, 60%, 45%))` }}>
           {initials}
         </div>
       )}
     </div>
     {editable && !isUploading && (
       <>
         <button className={styles.avatarEdit} onClick={() => setShowMenu(!showMenu)} title="Change photo">
           <PenIcon size={14} />
         </button>
         {showMenu && (
           <div className={styles.avatarMenu}>
             <button onClick={() => { onUpload(); setShowMenu(false); }}>
               Upload New
             </button>
             {src && (
               <button onClick={() => { onRemove(); setShowMenu(false); }} className={styles.dangerText}>
                 Remove Photo
               </button>
             )}
           </div>
     ...
     {editing ? (
       <div className={styles.efEdit}>
         ...
         <div className={styles.efActions}>
           <button className={`${styles.efBtn} ${styles.efBtnSave}`} onClick={save}>
             <CheckIcon />
           </button>
           <button className={`${styles.efBtn} ${styles.efBtnCancel}`} onClick={cancel}>
             <XIcon />
           </button>
         </div>
       </div>
 ...
 return (
   <div className={styles.card}>
     <div className={styles.cardImgWrap}>
       ...
       <span className={styles.cardBadge} style={{ background: "#1a1a2e", color: "#60a5fa" }}>
         BOUGHT
       </span>
     </div>
...
export default function Dashboard() {
 ...
 // The new Sold Modal state
 const [soldModal, setSoldModal] = useState({ isOpen: false, product: null, buyers: [], loading: false });
 ...
 const handleFileChange = async (event) => {
   ...
   try {
     ...
     const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/user/${userId}/avatar`, {
         method: "POST",
 ...
 const handleMarkSoldClick = async (product) => {
   ...
   try {
       const userId = localStorage.getItem("yahora_user_id");
       const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/messages/inbox/${userId}`);
       const result = await res.json();

       const productChats = (result.inbox || []).filter(chat => chat.product_id === product.id);

       setSoldModal({ isOpen: true, product, buyers: productChats, loading: false });
   } catch (error) {
 ...
 const executeMarkSold = async (buyerId = null) => {
   ...
   try {
     const token = localStorage.getItem("yahora_session");
     await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products/${productId}/sold`, {
       method: "POST",
       headers: {
         "Content-Type": "application/json",
         Authorization: `Bearer ${token}`,
 ...
 const handleToggleGridLike = async (productId, currentLikeState) => {
   ...
   try {
     await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products/${productId}/like`, {
         method: "POST",
 ...
 const handleToggleGridSave = async (productId, currentSaveState) => {
   ...
   try {
     await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products/${productId}/save`, {
         method: "POST",
         headers: { "Content-Type": "application/json" },
         body: JSON.stringify({ user_id: userId }),
     });
 ...
 const confirmDelete = (productId) => { setItemToDelete(productId); };
 ...
 return (
   <div className={styles.dashboardRoot} style={{ position: "relative", minHeight: "100vh", overflowX: "hidden" }}>
       ...
       <div className={styles.modalOverlay} onClick={() => !isDeleting && setItemToDelete(null)}>
         <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
           ...
           <div className={styles.modalActions}>
             <button className={styles.btnCancel} onClick={() => setItemToDelete(null)} disabled={isDeleting}>
               Cancel
             </button>
             <button className={styles.btnDelete} onClick={executeDelete} disabled={isDeleting}>
               <TrashIcon size={16} />
     ...
     {soldModal.isOpen && (
       <div className={styles.modalOverlay} onClick={() => setSoldModal({ isOpen: false, product: null, buyers: [], loading: false })}>
         <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
           <div className={styles.modalIconWrap} style={{ background: 'rgba(74, 222, 128, 0.1)', color: '#4ade80' }}>
             <CheckCircleIcon size={24} />
           </div>
           <h3 className={styles.modalTitle}>Mark as Sold</h3>
           <p className={styles.modalText} style={{ marginBottom: '1.5rem' }}>
             Who did you sell <strong>{soldModal.product?.title}</strong> to? Select from your active chats to build their purchase history.
           </p>

           <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '220px', overflowY: 'auto' }}>
             {soldModal.loading ? (
                 <div style={{ textAlign: 'center', padding: '1rem', color: '#888' }}>Loading active chats...</div>
             ) : (
                 <>
                   {soldModal.buyers.map((chat) => (
                       <button
                         key={chat.contact_id}
                         style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '100%', padding: '10px 14px', borderRadius: '8px', border: '1px solid #eaeaea', background: '#fff', cursor: 'pointer', textAlign: 'left' }}
                         onClick={() => executeMarkSold(chat.contact_id)}
                       >
                          <img src={chat.contact_avatar || "https://via.placeholder.com/32"} alt="Buyer" style={{ width: '36px', height: '36px', borderRadius: '50%', objectFit: 'cover' }} />
                          <div style={{ flex: 1, fontWeight: '600', color: '#1a1a2e', fontSize: '0.95rem' }}>{chat.contact_name}</div>
                       </button>
                   ))}
                   <button
                     style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '100%', padding: '10px 14px', borderRadius: '8px', border: '1px dashed #ccc', background: '#fafafa', cursor: 'pointer', textAlign: 'left', marginTop: '0.5rem' }}
                     onClick={() => executeMarkSold(null)}
                   >
                      <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#eee', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#888' }}>
                          <UserIcon size={18} />
                      </div>
                      <div style={{ flex: 1, fontWeight: '500', color: '#666', fontSize: '0.95rem' }}>Someone else (Off-platform)</div>
                   </button>
                 </>
             )}
           </div>
           ...
           <div className={styles.modalActions} style={{ marginTop: '1.5rem' }}>
             <button className={styles.btnCancel} onClick={() => setSoldModal({ isOpen: false, product: null, buyers: [], loading: false })}>
               Cancel
             </button>
           </div>
         </div>
     ...
     >
       ...
       <div className={styles.ui1Btn}>
         ...
         <button className={styles.sellBtn} style={{ width: "215px" }} onClick={() => navigate(`/user/${localStorage.getItem("yahora_user_id")}`)}>
       ...
       <div className={styles.sidebar}>
         <div className={styles.sidebarProfileCard}>
           ...
           <div className={styles.sidebarBio}>
             <EditableField value={profile.bio} placeholder="Tell the campus who you are…" textarea onSave={(v) => updateField("bio", v)} />
           </div>
         </div>
         ...
         <div className={styles.listNewCard}>
           <button className={styles.listNewIcon} onClick={() => { navigate("/sell"); window.scrollTo({ top: 0, left: 0, behavior: "auto" }); }}>
             <PlusIcon size={20} />
           </button>
         ...
         <div className={styles.grid} key={tab}>
             ...
             (listings.length ? (
               listings.map((item) => (
                 <ProductCard
                   ...
                   /* The newly added connections for the dashboard features */
               ...
               <div className={styles.emptyState}>
                 You haven't listed anything yet.<br />Hit the + button to start selling!
               </div>
             ))}
           ...
           {tab === "purchases" &&
             (purchases.length ? (
               ...
               <div className={styles.emptyState}>
                 You haven't bought anything yet.<br />Go explore the marketplace!
               </div>
             ))}
             
           {tab === "watchlist" && (
             <div className={styles.emptyState}>
               Your wishlist is empty.<br />Swipe right on items you love!
             </div>
           )}
         </div>
       </div>
     ...
     <div
       style={{
         ...
         opacity: isDashboard ? 1 : 0, transform: isDashboard ? "scale(1)" : "scale(0)",
         pointerEvents: isDashboard ? "all" : "none",
         transition: "opacity 0.4s ease, transform 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)",
       }}
     >
       <button className={styles.fab} title="List a new item" onClick={() => { navigate("/sell"); window.scrollTo({ top: 0, left: 0, behavior: "instant" }); }}>
         <PlusIcon size={24} />
       </button>
     </div>
   </div>
